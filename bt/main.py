            ...
            if file_size > 2 * 1024 * 1024 * 1024:
                ...
                if parts:
                    ...
                    for j, part_path in enumerate(parts, 1):
                        ...
                        part_caption = f"**File:** {filename}\n**Part {j} of {len(parts)}**\n**Size:** {get_readable_file_size(os.path.getsize(part_path))}"
                    ...
                    # If splitting failed, upload original
                    await progress_message.edit("**📤 Uploading file...**")
                    await message.reply_document(
                        result,
                        caption=f"**File:** {filename}\n**Size:** {get_readable_file_size(file_size)}",
                        progress=Leaves.progress_for_pyrogram,
                        progress_args=progressArgs("📤 Uploading", progress_message, time())
                    )
                    cleanup_download(result)
            else:
...
async def ytdlp_download_command(bot: Client, message: Message):
    ...
    for i, url in enumerate(urls, 1):
        try:
            ...
            success, result = await ytdlp_download(url, download_path, use_aria2c=True, progress_message=progress_message)
            ...
            file_size = os.path.getsize(result)
            LOGGER(__name__).info(f"Downloaded video size: {get_readable_file_size(file_size)}")
            
            # Determine if it's a video file
            from mimetypes import guess_type
            mime_type, _ = guess_type(result)
            is_video = mime_type and mime_type.startswith('video')
            
            # Check if file needs splitting (>2GB)
            if file_size > 2 * 1024 * 1024 * 1024:
                if is_video:
                    # Use existing video splitting method
                    ...
                    if parts:
                        ...
                        if parts:
                            for j, part_path in enumerate(parts, 1):
                                ...
                                await message.reply_document(
                                    part_path,
                                    caption=f"**Video Archive Part {j}/{len(parts)}**\nExtract all parts to get the video.",
                                    progress=Leaves.progress_for_pyrogram,
                            ...
                            # Upload as is
                            await _upload_video_or_doc(bot, message, result, url, progress_message)
                else:
...
async def _upload_video_or_doc(bot, message, file_path, source_url, progress_message):
    """Helper to upload video or document based on file type"""
    from mimetypes import guess_type
    from helpers.utils import get_media_info, get_video_thumbnail
    ...
    if mime_type and mime_type.startswith('video'):
        ...
        await message.reply_video(
            ...
            duration=duration,
            thumb=thumb,
            caption=f"**Source:** {source_url[:100]}...\n**Size:** {get_readable_file_size(file_size)}",
            progress=Leaves.progress_for_pyrogram,
